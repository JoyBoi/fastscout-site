...
const SEARCH_SYSTEM_PROMPT = `You validate vehicle data extracted from car trading platforms before it's used on AutoScout24.

AutoScout24 accepts these EXACT values:
BODY TYPES: Berline(6), Break(5), Cabriolet(2), Citadine(1), Coupé(3), Monospace(12), SUV/4x4/Pick-Up(4), Utilitaire(13), Autres(7)
...
Return JSON: {"status":"ok"|"warnings"|"errors","issues":[{"field":"<name>","severity":"error"|"warning","extracted":"<source value>","mapped":"<AS24 code or null>","expected":"<correct AS24 label>","message":"<explanation>"}],"confidence":0.0-1.0}

Rules:
- "error" = definitely wrong (BMW X5 mapped to Coupé → should be SUV)
- "warning" = suspicious but possible (Clio with 200kW)
- Only flag OBVIOUS issues. Don't guess. Empty issues array = data looks correct.
- Check: brand+model vs body type, fuel type vs model (Tesla=Electric), power range vs model class, transmission vs model (EV=Automatic), mileage vs year plausibility
- "UNMAPPED" in mapped field = our mapping couldn't find an AS24 code → flag as error with suggestion
- Messages in French, under 80 chars.`;

const MANUAL_LISTING_EXTRA = `
ADDITIONAL FIELDS for manual listing:
...
export const POST: APIRoute = async (ctx) => {
  ...
  try {
    ...
    const status = validStatus.includes(raw.status as string) ? raw.status : "ok";
