...
// ---------------------------------------------------------------------------
// POST /api/extension/auth?mode=signin
// Called by the Chrome extension popup to sign in with email + password.
// Validates credentials via Convex Auth and returns a Convex token.
// ---------------------------------------------------------------------------
export const POST: APIRoute = async (ctx) => {
  ...
  if (mode !== "signin") {
    return new Response(JSON.stringify({ error: "unsupported_mode" }), { status: 400, headers });
  }
